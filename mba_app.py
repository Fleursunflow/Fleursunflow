...
ourteam = Image.open('slmt-01.png')
st.image(ourteam)
with st.sidebar:
        selector = st.selectbox(
        label = "Halo selamat datang",
        options = ["Market Basket","aboutus"],
        )
if selector == "Market Basket":
        st.set_option('deprecation.showPyplotGlobalUse', False)
        file_bytes = st.file_uploader("Upload a file", type="csv")
        words = st.sidebar.selectbox("Silahkan masukan berapa jumlah jenis paket", range(1,1000,1))
        ...
        st.header("Halo salam kenal")
        ourteam = Image.open('abts-01.png')
        st.image(ourteam)
